...
export default function ItemRow({ item, index, onChange, onRemove, canRemove }) {
  ...
  return (
    <tr>
      ...
      <td><input type="number" min="0" value={tax} onChange={(e) => onChange(index, "tax", Math.max(0, e.target.value))} /></td>
      <td><input type="number" min="0" value={gst} onChange={(e) => onChange(index, "gst", Math.max(0, e.target.value))} /></td>
      <td><input type="number" min="0" value={vat} onChange={(e) => onChange(index, "vat", Math.max(0, e.target.value))} /></td>
      <td><input type="number" min="0" value={discount} onChange={(e) => onChange(index, "discount", Math.max(0, e.target.value))} /></td>
